  ...
  fetchPhotos() {
    ...
    this.library = [];
      ...
      this.photoLibrary.requestAuthorization({read: true}).then(() => {
        ...
        this.photoLibrary.getLibrary({ thumbnailWidth: THUMBNAIL_WIDTH, thumbnailHeight: THUMBNAIL_HEIGHT,itemsInChunk: 100,chunkTimeSec: 0.5}).subscribe({
        next: (chunk) => {
          this.library = this.library.concat(chunk);
          this.library = this.library.slice(0, 100); // To take top 10 images
...
alert(this.user.categoria)

  photo.seleccion=true
...
var random = Math.floor(Math.random()*100);
  ...
  let options: FileUploadOptions = {
     fileKey: 'photo',
     fileName: 'myImage_'+photo.id+'_mascojudo_'+this.user.categoria+'_mascojudo_'+this.usuario+".jpg",
     chunkedMode:false,
     httpMethod:'post',
     mimeType:"image/jpeg",
     headers: {}
